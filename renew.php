...
include('includes/twilio.php'); // Include Twilio messaging
...
{
    global $conn;
    $currencyQuery = "SELECT currency FROM settings"; // Assuming currency is stored in 'settings' table
    $currencyResult = $conn->query($currencyQuery);

    if ($currencyResult->num_rows > 0) {
        $currencyRow = $currencyResult->fetch_assoc();
        return $currencyRow['currency']; // Example: ₹, $, €, etc.
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateMemberQuery = "UPDATE members SET membership_type = $membershipTypeId, expiry_date = '$expiryDate', message_sent = 0 WHERE id = $memberId";
    $updateMemberResult = $conn->query($updateMemberQuery);

    // Insert into renewal history
    ...
    if ($updateMemberResult && $insertRenewResult) {
        ...
        // Send WhatsApp message after renewal
        if (!empty($memberDetails['contact_number'])) {
            $contactNumber = $memberDetails['contact_number'];
            if (substr($contactNumber, 0, 1) !== "+") {
                $contactNumber = "+91" . $contactNumber; // Modify country code if needed
            }

            $message = "Hello {$memberDetails['fullname']}, your gym membership has been successfully renewed! Your new expiry date is {$expiryDate}. Thank you for choosing us!";
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-6">
                                            ...
                                            <input type="text" class="form-control" id="mm" name="mm"
                                                value="<?php echo $memberDetails['membership_number']; ?>" disabled>
                                        </div>
                                    </div>
